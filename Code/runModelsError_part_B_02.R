...
dist$dist_deploy <- raster::extract(distCol, dist)
...
## no need to pass all distances in our dist$dist_deploy data.frame, just pass a representative 
##  range (take sample quantiles to get even coverage across our dist values)
qsamp <- seq(from = 0, to = 1, length.out = 1001)
temp <- data.frame(dist_deploy = quantile(na.omit(dist$dist_deploy), qsamp))
temp.scam <- gamsim::get_draws(object = scamMod, N = K, newdata = temp, enforce = "decreasing")
## so each column in temp.scam defines a response curve drawn from the fitted model
## apply each of these curves to our actual distances in dist$dist_deploy via approx
## I do not deny that this is inelegant, even by my low, low standards - BR
pred.scam <- do.call(cbind, lapply(seq_len(K), function(z) plogis(approx(x = temp$dist_deploy, y = temp.scam[, z], xout = dist$dist_deploy)$y)))
...
if (FALSE) {
  ## visual check
  dist$av <- pred.scam[, 1]
  plot(rasterFromXYZ(dist[, c("x", "y", "dist_deploy")]), main = "dist_deploy")
...
# pred.scam <- readRDS(paste0("../predictionsError/", this.species, "_", this.stage, "_scam_out.RDS"))
  ...
  pred.scam <- matrix(1, nrow = nrow(out), ncol = ncol(out))
}
