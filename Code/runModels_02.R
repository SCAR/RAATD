...
tune <- TRUE # Tune the model, or use fixed parameters?
tooBig <- FALSE # If the data are too large, sample only a proportion thereof
...
dat <- prepCaret(usage = usage, toobig = tooBig, prp = 0.1) # toobig allows sumbsampling of massive data frames
...
summary(gbmMod)
varImp(gbmMod)
saveRDS(gbmMod, paste0("fittedMods/", this.species, "_", this.stage, "_gbm.RDS"))
