...
# 
# writeRaster(existing_rast, "raster_mpa_existing.grd", format = "raster", overwrite = T)
existing_rast <- raster("raster_mpa_existing.grd")
...
# 
# writeRaster(proposed_rast, "raster_mpa_proposed.grd", format = "raster", overwrite = T)
proposed_rast <- raster("raster_mpa_proposed.grd")
...
# 
# writeRaster(eez_rast, "raster_eez.grd", format = "raster", overwrite = T)
eez_rast <- raster("raster_eez.grd")
...
# 
# writeRaster(ccamlr_rast, "ccamlr_rast.grd", format = "raster", overwrite = T)
ccamlr_rast <- raster("ccamlr_rast.grd")
...
plotGraph <- function(this.data, make.plot = TRUE) {
  ...
  return(hold) # Or return the data
  }
...
plotJurisdiction <- function(this.data, make.plot = TRUE) {
...
p <- plotGraph(this.data = datW, make.plot = TRUE)

pdf("mpaPropWeighted.pdf", width = 4.5, height = 2)
print(p)
dev.off()

# Get data to write csv for paper
mpaDat <- plotGraph(this.data = datW, make.plot = FALSE)
write.csv(mpaDat, "Data_Fig3b.csv", row.names = F)
...
p <- plotGraph(this.data = datNW, make.plot = TRUE)
...
p <- plotJurisdiction(this.data = datW, make.plot = TRUE)
...
# Get data to write .csv for paper
jurisdictionDat <- plotJurisdiction(this.data = datW, make.plot = FALSE)
write.csv(jurisdictionDat, "Data_Fig3c.csv", row.names = F)
...
plot(d,
     ...
     horizontal = TRUE,
     legend.shrink=0.7,
     legend.args = list(text = 'Mean habitat importance')
)
dev.off()
